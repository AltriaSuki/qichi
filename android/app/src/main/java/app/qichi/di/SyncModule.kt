...
object SyncModule {
    ...
    @Provides
    @Singleton
    fun accountRepository(db: QichiDatabase, api: ApiClient, session: SessionManager, rooms: RoomRepository): AccountRepository =
        AccountRepository(db, api, session, rooms)
    ...
    @Provides
    @Singleton
    fun decisionRepository(db: QichiDatabase, store: LocalStore, scheduler: SyncScheduler, session: SessionManager): DecisionRepository =
        DecisionRepository(db, store, scheduler, session)
