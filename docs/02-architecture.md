...
两台手机没有谷歌服务（2026-09-23 人类确认）。**默认用内置通知**（D12）：App 在后台由前台服务（类型 remoteMessaging）保持实时连接，服务端经 WebSocket 发 `notify`；UnifiedPush + 自建 ntfy（D10）作为备选；`fcm` 保留在枚举里，服务端配置它会报错。推送像 QQ 那样带内容（标题是谁，正文是消息原文、心情的几句、待办标题等，最多 300 字；只经过自建的 ntfy），每人可以在「通知」里关掉「通知里显示内容」，关掉后只有「谁做了什么」；按每人的通知偏好和免打扰时段（房间时区）过滤；推送地址失效（404 / 410）时自动删除该设备。
...
| D13 | 界面换成「新方向」：正文改用思源黑体、数字用 IBM Plex Mono、手写短句用龙藏体（只保留用到的字），思源宋体只留给书页；删掉 Cormorant Garamond（2026-09-24，人类选定新方向） | 字和层次参考 Day One、iA Writer、Bear，更好读；代价是安装包约多 23MB（黑体约 18MB、宋体保留约 24MB）。如果嫌大，可以让书页也改用黑体、删掉宋体，省约 24MB |
