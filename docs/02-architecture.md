...
两台手机没有谷歌服务（2026-09-23 人类确认），**只实现了 UnifiedPush**（见 D10）；`fcm` 保留在枚举里，服务端配置它会报错。推送像 QQ 那样带内容（标题是谁，正文是消息原文、心情的几句、待办标题等，最多 300 字；只经过自建的 ntfy），每人可以在「通知」里关掉「通知里显示内容」，关掉后只有「谁做了什么」；按每人的通知偏好和免打扰时段（房间时区）过滤；推送地址失效（404 / 410）时自动删除该设备。
