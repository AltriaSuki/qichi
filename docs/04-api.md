...
| GET | `/rooms/{roomId}/on-this-day?date=` | 某一天聊天里发过的照片（「一年前的今天」用，灵感、决定、心情在手机上本来就有） |
...
| 方法 | 路径 | 说明 |
|---|---|---|
| GET | `/rooms/{roomId}/export?files=` | 房间数据导出为 ZIP（说明.txt、data.json、聊天记录.md、文稿/，`files=true` 时加 附件/）；不含撤回、回收站里的内容，也不含对方没共享或还没揭晓的内容 |
