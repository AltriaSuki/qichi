...
object SummaryData {
    ...
    fun gather(roomId: UUID, start: LocalDate, end: LocalDate, zone: ZoneId, names: Map<UUID, String>, prefs: AiPrefs = AiPrefs()): List<SourceLine> {
        ...
        if (prefs.ideas) raw += Ideas.selectAll().where { (Ideas.roomId eq roomId) and Ideas.deletedAt.isNull() and (Ideas.createdAt greaterEq from) and (Ideas.createdAt less until) }
            .map { Raw(EntityType.Idea, it[Ideas.id], it[Ideas.createdAt], it[Ideas.authorId], "灵感", it[Ideas.body]) }
        if (prefs.plans) raw += Plans.selectAll().where {
        ...
        if (prefs.archive) raw += ArchiveItems.selectAll().where { (ArchiveItems.roomId eq roomId) and ArchiveItems.deletedAt.isNull() and (ArchiveItems.updatedAt greaterEq from) and (ArchiveItems.updatedAt less until) }
