    ...
    suspend fun list(userId: UUID, roomId: UUID, cursorText: String?, limit: Int): TrashPage {
        ...
        return db.tx(readOnly = true) {
            ...
            val candidates = buildList {
                ...
                Plans.selectAll().deleted(Plans, roomId, cursor, take).forEach { row ->
                    val p = row.toPlan()
                    add(Candidate(TrashType.Plan, p, p.deletedAt!!, p.deletedBy!!))
                }
                Ideas.selectAll().deleted(Ideas, roomId, cursor, take).forEach { row ->
                    val i = row.toIdea()
                ...
                ArchiveItems.selectAll().deleted(ArchiveItems, roomId, cursor, take).forEach { row ->
        ...
        db.tx {
            ...
            when (type) {
                TrashType.Message -> {
                    ...
                    // 以它为来源的档案条目：来源断开（外键也会置空，这里写变化让客户端同步到）
                    ArchiveItems.select(ArchiveItems.id).where { ArchiveItems.sourceMessageId eq id }.map { it[ArchiveItems.id] }.forEach { item ->
                        writes.update(this, roomId, userId, EntityType.ArchiveItem, item, ArchiveItems) { it[ArchiveItems.sourceMessageId] = null }
                    }
                ...
                TrashType.Plan -> {
                    ...
                    Milestones.select(Milestones.id).where { Milestones.planId eq id }.map { it[Milestones.id] }
                        .forEach { hardDelete(this, roomId, userId, EntityType.Milestone, it, Milestones, now) }
                    PlanStages.select(PlanStages.id).where { PlanStages.planId eq id }.map { it[PlanStages.id] }
                        .forEach { hardDelete(this, roomId, userId, EntityType.PlanStage, it, PlanStages, now) }
