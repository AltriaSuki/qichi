...
| 文件 | 做什么 | 用法 |
|---|---|---|
| `uix.py` | 操作模拟器：列出屏幕上的文字、按文字点击、输入英文、截图、无障碍检查（小于 44dp 的可点区域、读屏会念的字） | `python3 tools/uix.py texts`、`python3 tools/uix.py tap 聊天`、`python3 tools/uix.py shot 截图.png`、`python3 tools/uix.py a11y`；也可以在脚本里 `from uix import *` |
...
| `ai_ask.py` | 以某人身份问 AI 一组问题，列出回答、引用、用量、耗时和它查了什么（会花 AI 用量，只用在本机） | `python3 tools/ai_ask.py aqi password-aqi "这周有什么安排？" "预算多少？"` |
